    ...
    if (values.userName && values.phoneNumber) {
      ...
      result
        ? localStorage.setItem(
            "allUsers",
            JSON.stringify([{ ...result, isOpen: true }])
          )
        : localStorage.setItem(
            "allUsers",
            JSON.stringify([{ ...User, ...values, isOpen: true }])
          );
      localStorage.setItem(
        "user",
        JSON.stringify({ ...User, ...values, isOpen: true })
      ...
      <div className={styles.containerLogin}>
        <div className={styles.wrapLogin100}>
          ...
          >
            ...
            >
              <Grid item>
                <Typography className={styles.labelInput100}>
                  Username
                </Typography>

                <TextField
                  required
                  defaultValue={textInput.userName}
                  name="userName"
                  variant="outlined"
